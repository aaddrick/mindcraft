...
import { HuggingFace } from '../models/index.js';
...
export class Prompter {
    ...
    async promptConvo(messages) {
        const current_msg_time = Date.now();
        this.most_recent_msg_time = current_msg_time;
        ...
        for (let i = 0; i < 3; i++) { // try 3 times to avoid hallucinations
            ...
            // Check if a new message arrived while we were generating
            if (this.most_recent_msg_time > current_msg_time) {
                console.log('New message arrived while generating - discarding response');
                return null;
            }
    ...
    async promptCoding(messages) {
        if (this.awaiting_coding) {
            console.log('Already processing a coding request - skipping');
            return null;
        }
